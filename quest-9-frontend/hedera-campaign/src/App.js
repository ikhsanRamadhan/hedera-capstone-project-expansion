...
function App() {
  ...
  const connect = async () => {
    if (window.ethereum) {
      ...
      if(address.toLowerCase() !== merchantAddress.toLowerCase()) {
        let present = await contractInstance.checkUserExists( address );
        let username;
        if ( present )
            username = await contractInstance.getUsername( address );
          else {
            username = prompt("Enter a username", "Guest"); 
            if( username === "" ) username = "Guest";
            await contractInstance.addRenter(address, username, true, false, 0, 0, 0, 0);
          }
  ...
  const borrowCar = async (tokenSolidityAddress, serial) => {
    ...
    try {
      ...
      const tx = await contract.borrowing(tokenSolidityAddress, defaultAccount, serial, {
      ...
      alert("Failed to borrrow car (You have a pending due)");
      console.log(e);
    }
  };
  ...
  const returnCar = async (tokenSolidityAddress, serial) => {
    try {
      ...
      // Part 14 - return car
      const tx = await contract.returning(tokenSolidityAddress, defaultAccount, serial, {
        gasLimit: 1_000_000,
      });
      await tx.wait();
      ...
      <Routes>
        {isMerchant ? (
          ...
          <>
            ...
            <Route path="/give" element={<Return returnCar={returnCar} address={defaultAccount} myContract={contract}/>} />
          </>
        ) : (
          <></>
        )}
